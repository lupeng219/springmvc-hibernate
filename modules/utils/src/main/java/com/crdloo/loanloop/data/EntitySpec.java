...
public class EntitySpec<T> implements Specification<T> {
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (orEntity != null) {
            List<Predicate> orPredicates = addPredicate(new ArrayList<Predicate>(), root, orEntity,
                    cb);
    ...
    protected List<Predicate> addPredicate(List<Predicate> predicates, Path<?> root, Object obj,
            CriteriaBuilder cb) {
        PropertyDescriptor[] fields = BeanUtils.getPropertyDescriptors(obj.getClass());
                ...
                if (value != null && !Class.class.isAssignableFrom(clazz)) {
                    ...
                    if (String.class.isAssignableFrom(clazz)) {
                        if (!StringUtils.isBlank(value.toString())) {
                            predicates.add(cb.like((Path<String>) path, value.toString()));
                        }
    ...
    protected Predicate orTogether(List<Predicate> predicates, CriteriaBuilder cb) {
