...
public class JsonHelper {
    ...
    public static <T> T extend(Class<T> target, Object root, Object... args) {
        // Gson gson = new Gson();
        Gson gson = new GsonBuilder().registerTypeAdapter(Timestamp.class, new TimestampAdapter())
                .create();
    ...
    public static <T> T convert(JsonElement source, Class<T> target) {
        // Gson gson = new Gson();
        Gson gson = new GsonBuilder().registerTypeAdapter(Timestamp.class, new TimestampAdapter())
                .create();
    ...
    public static JsonObject i10n(JsonObject object, Locale locale) {
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            ...
            if (name.endsWith(localeName)) {
                String i10nName = name.substring(0, name.length() - localeName.length());
