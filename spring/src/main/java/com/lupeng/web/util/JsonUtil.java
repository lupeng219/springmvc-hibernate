...
public class JsonUtil {
	...
	/**
	 * @desc json字符串转换为指定的class对象。其中以class中的field为准，如果json中没有此field，
	 *       class对象中此field忽略 第一次调用的class，会解析class field信息，放入内存，外后都是从内存获取
	 * @param jsonStr
	 *            json字符串
	 * @param clazz
	 *            目标对象class，此对象及里面的对象必须拥有默认的构造函数
	 */
	...
	/**
	 * 重载方法 Map<String, Object> json2map(String jsonStr, String fieldName,Class
	 * <?> clazz) fieldname="data"
	 * 
	 */
	...
	public static Object toObject(String json, Class<?>... returnClass) {
		...
		if (outputDataClass.equals(String.class)) {
			...
			try {
				if (returnClass.length == 1)
					dataValue = OBJECT_MAPPER.readValue(json, outputDataClass);
				else {
					Class[] paraCalss = new Class[returnClass.length - 1];
					System.arraycopy(returnClass, 1, paraCalss, 0, returnClass.length - 1);
					dataValue = OBJECT_MAPPER.readValue(json, TypeFactory.parametricType(outputDataClass, paraCalss));
				}
			} catch (Exception e) {
				logger.warn(e.getMessage(), e);
			}
